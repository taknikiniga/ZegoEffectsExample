...
class ZegoExpressRangeSceneStreamImp : public IZegoRangeSceneStream {
  public:
    ...
    int setReceiveRange(float range) override {
        return zego_express_range_scene_stream_set_receive_range(rangeSceneHandle, range);
    }
    ...
    int updatePlayingCanvas(const std::string &streamID, ZegoCanvas *canvas) override {
        ...
        if (canvas == nullptr) {
            error = oInternalOriginBridge->updatePlayingCanvas(rangeSceneHandle, streamID.c_str(),
                                                               nullptr);
        } else {
